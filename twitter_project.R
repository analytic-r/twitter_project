...
# Make tweets plain words with alphabetic character set.
...
# Define an evaluator function whick matches the tweets' words with sentiment words and add values
...
labs(title = "Daredevil", x = "Time", y = "Average Sentiment Score") + ylim(min(long$value), max(long$value)) +
theme_bw() + facet_grid(variable ~ .)



# Remove words from the tweets and create a wordcloud
...
# Extract locations of the twitters
locations <- matrix()
...
location_matrix <- data.frame(names = tw_names[1:1001], locations = na.omit(locations), row.names = NULL, stringsAsFactors = FALSE)
